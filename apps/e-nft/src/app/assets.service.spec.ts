...
describe('AssetsService', () => {
  ...
  it('should returnassets collection', waitForAsync(() => {
    service.getAssets().subscribe(assets => {
      expect(assets.length).toBe(3);
      expect(assets[0].tokenId).toBe(RESPONSE_MOCK.assets[0].token_id);
    });

    const expectedUrl = `${environment.openseaApi}/assets?collection=zuzana-breznanikova-art`;
